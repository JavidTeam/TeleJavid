...
-----------------------------------------
local function delmsg (TeleJavid,TeleJavidBot)
    msgs = TeleJavid.msgs 
    for k,v in pairs(TeleJavidBot.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(TeleJavidBot.messages_[0].chat_id_,{[0] = TeleJavidBot.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(TeleJavidBot.messages_[0].chat_id_, TeleJavidBot.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
-----------------------------------------
local function Javid(msg)
