--Begin Tools.lua :)
local SUDO = 226399128 -- put Your ID here! <===
		...
		if ((matches[1] == 'charge' and not Clang) or (matches[1] == "شارژ" and Clang)) and matches[2] and not matches[3] and is_sudo(msg) then
			if tonumber(matches[2]) > 0 and tonumber(matches[2]) < 1001 then
				...
				if lang then
					...
					tdcli.sendMessage(SUDO, 0, 1, 'ربات در گروه '..msg.to.id..' به مدت `'..matches[2]..'` روز تمدید گردید.', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, 'ربات با موفقیت تنظیم شد\nمدت فعال بودن ربات در گروه به '..matches[2]..' روز دیگر تنظیم شد...', 1, 'md')
...
local text = matches[2]
tdcli.createNewChannelChat(text, 1, '@Javid_Team', (function(b, d) tdcli.addChatMember(d.id_, msg.from.id, 0, dl_cb, nil) end), nil)
   if not lang then 
...
if ((matches[1] == 'Javid' and not Clang) or (matches[1] == "javid" and Clang)or (matches[1] == "جاوید" and Clang)) then
...
_Sudoer And Admins Javid Bot Help :_
...
text = [[
_راهنمای ادمین و سودو های ربات جاوید:_
